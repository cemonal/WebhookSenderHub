...
namespace WebhookSenderHub.JsonConverters
{
	...
	internal class JsonStringEnumMemberConverterOptions
    {
        ...
        private static ulong? ConvertEnumValueToUInt64(object deserializationFailureFallbackValue)
        {
            ...
            switch (deserializationFailureFallbackValue)
            {
                case int intVal:
                    value = (ulong)intVal;
                    break;
            ...
            if (value == null)
            {
                if (deserializationFailureFallbackValue is Enum enumValue)
                    value = JsonStringEnumMemberConverter.GetEnumValue(Type.GetTypeCode(enumValue.GetType()), deserializationFailureFallbackValue);
                else
